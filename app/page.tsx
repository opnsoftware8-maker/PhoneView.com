...
export default function App() {
  ...
  useEffect(() => {
      ...
      const FilteredData = Allphone.xiaomiphone.filter(phone => {
          ...
          for (const key in RecChoos) {
              ...
              if (RecChoos[key] === true && phone.recommended[key] === false) {
                  return false; 
              }
          }
          return true; 
  ...
  return (
    <div>
      <header className="w-full bg-zinc-900/100 text-white rounded-none px-6 py-6 border-b border-neutral-800">
      ...
      <main className="p-4">
        <div>
          ...
          <div>
            {!filterbutton ? (
              <button onClick={() => setFilterbutton(true)} className="bg-gray-200 p-2 text-black">คัดกรอง ↓</button>
            ) : (
              <div className="border p-4 bg-gray-50 text-black">
                <button onClick={() => setFilterbutton(false)} className="mb-2 font-bold">คัดกรอง ↑</button>
                <div>
                  {!RecChoos.activate ? (
                    <button onClick={() => SetRecChoos({ ...RecChoos, activate: true })}>ประเภทการใช้งาน ↓</button>
                  ) : (
                    <div className="flex flex-col gap-2">
                      <button onClick={() => SetRecChoos({ ...RecChoos, activate: false })} className="font-bold text-blue-600"><span>ประเภทการใช้งาน ↑</span></button>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {PhoneArr.length >= 1 && PhoneArr.map((Data, index) => (
            <Phonecard key={index} Device={Data} />
          ))}
        </div>
...
ี่แก้ไวยากรณ์แล้ว พร้อมใส่ Grid ให้การ์ดเรียงกันสวยๆ */}
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {PhoneArr.length >= 1 && PhoneArr.map((Data, index) => (
            <Phonecard key={index} Device={Data} />
          ))}
